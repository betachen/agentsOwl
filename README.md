...
ff r               # peer 会话（含固定 pair）：编号选择后进入并显示最近上下文
...
按 `q` 返回；`ff r` 对已经完成的 peer 会自动重启同一 rollout，并在进入时显示最近约
四轮问答；完整历史仍可按 `Ctrl+T` 打开。这样只优化可见上下文，不截断原生会话历史，
也不会创建新会话。对仍在执行的 peer 则只 attach，不会打断任务。这只是显示方式，
不表示上下文丢失。
