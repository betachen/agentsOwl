...
发送时用可选的 `--focus` 指定本轮问题、范围和表达方式：

```bash
ff send-peer range-v01-r2 --focus '请用中文回答：
1. 本轮方案能否进入下一步？依据是什么？
2. 哪些问题现在必须处理，哪些可以推迟？
先逐项给结论和理由，仅附必要证据路径。不要修改项目代码或替我作决策。'
```

这些要求与 worker handoff 分开放进请求，并记录在 `events.jsonl` 的对应
`send-peer` 事件中。Peer 会被要求先回答问题，将通用回复模板作为检查清单，
不必机械填写固定章节。证据和影响结论的未验证项仍需说明，项目规则仍然有效。
`--focus` 是可选项；省略时保留原来的通用 review。兼容别名 `send-review`
也支持该参数。

每次发送仍需有新的 `inbox/worker-handoff.md`。如果只想追问已经完成的 review，
...
- **关键发现**：问题、影响、必要证据和对应建议放在一起，区分明确错误与可选改进。
- **验证与局限**：简述检查结果及影响结论的未验证项、假设和不确定性。

空章节可以省略，不要求重复治理声明或罗列所有读过的文件。提供 `--focus`
时仍优先按你的问题组织回复。模板调整只影响后续生成的回复，不会改写已有文件。
